...
When an uploaded lesson plan strongly matches Filipino or Araling Panlipunan markers, such as `Araling Panlipunan`, `Aral-Pan`, `Asignatura: Filipino`, `Layunin`, `Kasanayang Pampagkatuto`, `Gawain`, `Pagtataya`, or `Takdang-Aralin`, the app switches the output language to Filipino before generation. It does not auto-switch back to English from uploaded content, so teachers can still intentionally generate Filipino decks from English source material.
